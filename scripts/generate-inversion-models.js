 ...
 */
export const create${modelId}Model = (config: Partial<InversionConfig> = {}) => {
...
const readmeTemplate = (modelId) => `# ${modelId}: Inversion Model
...
// Generate models IN3-IN20
async function generateInversionModels() {
  const startTime = Date.now();
  const models = [];
  
  // Generate model IDs IN3 through IN20
  for (let i = 3; i <= 20; i++) {
    const modelId = `IN${i.toString().padStart(2, '0')}`;
    models.push(modelId);
  }
  
  console.log(`Generating ${models.length} Inversion models (IN3-IN20)...`);
...
async function run() {
  try {
    await generateInversionModels();
    console.log('\n🎉 Inversion model generation complete!');
